...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, itemClassName = '' }) => (
    <div
        className={`scroll-stack-card relative w-full h-[600px] md:h-[80vh] my-8 p-0 rounded-[40px] shadow-[0_0_30px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform ${itemClassName}`.trim()}
        style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
    ...
    const updateCardTransforms = useCallback(() => {
        ...
        cardsRef.current.forEach((card, i) => {
            ...
            if (blurAmount) {
                let topCardIndex = 0;
                // Optimization: Don't loop efficiently if valid window scroll
                // Simple logic: if pinned, maybe add blur? 
                // Or stick to user logic:
    ...
    useLayoutEffect(() => {
        ...
        cards.forEach((card, i) => {
            ...
            // Use standard translate3d for GPU accel
    ...
    return (
        <div
            ...
            ref={scrollerRef}
        >
            <div className={`scroll-stack-inner relative ${useWindowScroll ? '' : 'pt-[20vh] px-20 pb-[50rem] min-h-screen'}`}>
                {children}
